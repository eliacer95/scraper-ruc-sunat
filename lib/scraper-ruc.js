...
function getExtendedInformation(html , callback) {
	try {
		...
		for (var i = 0; i < ec_act.length; i++) {
			...
			array_ac_act.push(ec_act[i].split('- ')[1]);
		}

		contributor.economic_activities = array_ac_act;

		contributor.authorized_vouchers			= table.eq(10+leap).children().eq(1).children().eq(0).children().map(function () {
			return $(this).text().trim();
		}).get();
		...
		contributor.standards   			= table.eq(15+leap).children().eq(1).children().eq(0).children().map(function () {
			return $(this).text().trim();
		}).get();
...
function getHistoricalInformation(html , callback) {
	try {
		var $	             				= cheerio.load(html);
		var table = $('table:nth-of-type(2) tr td tr td table');
...
function getProbatoryActs(html , callback) {
	try {
		...
		for (var i = 1; i < table.children().length; i++) {
			var actPro			= {};
			if (table.children().eq(i).children().length > 1) {
...
Scraper.prototype.getInfHis = function (ruc ,desRuc, cb) {
	...
	getHtmlPageDetailed(ruc, desRuc, action,function (err , body) {
		...
		getHistoricalInformation(body , function (err , data) {
			if ( err ) {
				return cb(err);
			} else {
				return cb(null , data , body);
...
Scraper.prototype.getInfoDC = function (ruc ,desRuc, cb) {
	
	var action = 'getInfoDC';
	
	getHtmlPageDetailed(ruc, desRuc, action,function (err , body) {
		if ( err ) {
			return cb(err);
		}
		getCoactiveDebt(body , function (err , data) {
			if ( err ) {
				return cb(err);
			} else {
				return cb(null , data , body);
...
Scraper.prototype.getInfoOT = function (ruc ,desRuc, cb) {
	
	var action = 'getInfoOT';
	
	getHtmlPageDetailed(ruc, desRuc, action,function (err , body) {
		if ( err ) {
			return cb(err);
		}
		getTaxOmissions(body , function (err , data) {
			if ( err ) {
				return cb(err);
			} else {
				return cb(null , data , body);
