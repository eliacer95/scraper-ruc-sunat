...
function getExtendedInformation(html , callback) {
	try {
		...
		contribuyente.actividades_economicas   	= table.eq(8).children().eq(1).children().eq(0).children().map(function () {
			return $(this).text().trim();
		}).get();
		contribuyente.comprobantes_autorizados 	= table.eq(9).children().eq(1).children().eq(0).children().map(function () {
			return $(this).text().trim();
		}).get();
		contribuyente.comprobantes_electronicos	= table.eq(10).children().eq(1).text().trim();
		contribuyente.fecha_inscripcion	   	 	= table.eq(11).children().eq(1).text().trim();
		contribuyente.padrones   				= table.eq(12).children().eq(1).children().eq(0).children().map(function () {
			return $(this).text().trim();
		}).get();
