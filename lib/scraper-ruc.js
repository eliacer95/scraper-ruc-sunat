...
function getBasicInformation(html , callback) {
	...
	try {
		...
		if(contributor.contributor_type.search("PERSONA") > -1 || aux.search("DNI") > -1){
			...
			contributor.document_type 		= personal_information[0].trim();
		...
		var objeto = {
			...
			document_type       : clean(contributor.document_type.toString()),
        	owner               : clean(contributor.owner.toString()),
...
function getExtendedInformation(html , callback) {
	try {
		...
		var objeto = {
			...
			authorized_vouchers	: clean(contributor.authorized_vouchers.toString()),
			...
			standards			: clean(contributor.standards.toString())
		}
		return callback(null , objeto , html);
	} catch ( e ) {
		return callback(e);
