...
function getBasicInformation(html , callback) {
	...
	try {
		...
		var aux						   		= table.eq(3).children().eq(1).text().trim();
		...
		contributor.business_name   		= table.eq(3+salto).children().eq(1).text().trim();
		contributor.registration_date  		= table.eq(4+salto).children().eq(1).text().trim();
		contributor.status               	= table.eq(5+salto).children().eq(1).text().trim();
		contributor.condition            	= table.eq(6+salto).children().eq(1).text().trim();
		contributor.fiscal_address		   	= table.eq(7+salto).children().eq(1).text().trim();

		var objeto = {
			ruc 				: clean_ruc(contributor.ruc.toString()),
			social_reason		: clean_data(contributor.social_reason.toString()),
			contributor_type	: clean_data(contributor.contributor_type.toString()),
			document_type       : clean_data(contributor.document_type.toString()),
        	owner               : clean_data(contributor.owner.toString()),
			business_name		: clean_data(contributor.business_name.toString()),
			registration_date	: clean_data(contributor.registration_date.toString()),
			status 				: clean_data(contributor.status.toString()),
			condition			: clean_data(contributor.condition.toString()),
			fiscal_address		: clean_data(contributor.fiscal_address.toString())
		}
...
function getExtendedInformation(html , callback) {
	try {
		...
		contributor.electronic_vouchers			= table.eq(10+salto).children().eq(1).text().trim();
		contributor.ple_affiliation		   	 	= table.eq(11+salto).children().eq(1).text().trim();
		contributor.standards   				= table.eq(12+salto).children().eq(1).children().eq(0).children().map(function () {
			return $(this).text().trim();
		}).get();

		var objeto = {
			economic_activities	: clean_data(contributor.economic_activities.toString()),
			authorized_vouchers	: clean_data(contributor.authorized_vouchers.toString()),
			electronic_vouchers	: clean_data(contributor.electronic_vouchers.toString()),
			ple_affiliation		: clean_data(contributor.ple_affiliation.toString()),
			standards			: clean_data(contributor.standards.toString())
		}
		return callback(null , objeto , html);
	} catch ( e ) {
		return callback(e);
