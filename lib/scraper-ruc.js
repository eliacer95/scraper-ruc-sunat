...
function getBasicInformation(html , callback) {
	...
	try {
		...
		console.log("coincidencia:",contributor.contributor_type.search("PERSONA"));
		if(contributor.contributor_type.search("PERSONA")=! -1 || contributor.business_name.search("DNI")!= -1){
			personal_information 			= contributor.business_name.split("-");
